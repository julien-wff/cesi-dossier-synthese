...
func extractGrades(pageSquares []*PageSquares) ([]Section, *utils.APIError) {
	...
	if len(squares) < 2 {
		return []Section{}, utils.NewGradesExtractionError("not enough squares")
	}
	...
	if len(sectionsIndexes) == 0 {
		return []Section{}, utils.NewGradesExtractionError("no sections found")
	}
